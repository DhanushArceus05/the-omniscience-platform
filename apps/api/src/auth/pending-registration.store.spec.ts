...
describe("PendingRegistrationStore", () => {
  ...
  describe("get", () => {
    it("returns null when no record exists", async () => {
      ...
      expect(get).toHaveBeenCalledWith("auth:pending-registration:user@example.com");
    });
  ...
  describe("claimSend", () => {
    it("runs the claim script with the key, cooldown, ttl and JSON-encoded record", async () => {
      ...
      expect(evalFn).toHaveBeenCalledWith(
        expect.any(String),
        1,
        "auth:pending-registration:user@example.com",
        60,
        600,
        expect.any(Number),
  ...
  it("deletes a record by key", async () => {
    ...
    expect(del).toHaveBeenCalledWith("auth:pending-registration:user@example.com");
  });
});
