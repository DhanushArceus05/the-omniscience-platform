# Phase 5 — OmniCore (previously Phase 4 — OmniProvider & Model Manager)

## Current status (updated — see the "Phase 5" step sections at the end of this file for full
detail)
...
- **Phase 4 — OmniProvider & Model Manager**: complete. All 5 steps (Provider Foundation & Domain
  Architecture, Anthropic Real Execution, `POST /ai/generate`, Google Gemini Real Execution,
  Production Hardening) are implemented and verified — see the dedicated Phase 4 sections below.
- **Phase 5 — OmniCore, Step 1 (OmniCore Foundation)**: complete — see the dedicated section at the
  end of this file.
- **Phase 5 — OmniCore, Step 2 (Intent Intelligence)**: complete — see the dedicated section at the
  end of this file.
...
- **Phase 6 — Omniscience Assistant, Step 4 (Conversation Management)**: COMPLETE — implementation,
  local verification (including the real-Mongo repository spec, 19/19, on the user's machine),
  manual browser QA, commit, and push are all done and the push has been explicitly confirmed by
  the user — see `## Phase 6 — Omniscience Assistant, Step 4 (Conversation Management)` near the
  end of this
...
- **Phase 6 — Omniscience Assistant, Step 5 (Message-Level UX)**: COMPLETE — see
  `## Phase 6 — Omniscience Assistant, Step 5 (Message-Level UX)` near the end of this file. Adds
  the guarded last-message-only delete primitive, regenerate-last-assistant-response, and
  edit-and-resend-last-user-message; message-level copy was found to already exist from Step 3 and
  was deliberately not reimplemented. Includes a critical bugfix round (full-history pagination in
  `ConversationThread.tsx`, URL-based conversation-selection persistence) found and fixed via the
  user's own real browser QA before any commit. Implementation, full in-sandbox verification, the
  bugfix round, and the user's own final local QA are all done and confirmed.
- **Phase 6 — Omniscience Assistant, Step 6 (Phase Close-Out)**: now the active/current step — not
  yet started, awaiting explicit go-ahead before implementation begins. See
  `claude/PHASE_PLAN.md`'s Step 6 section for the approved scope (documentation reconciliation
...
- **Phase 4 — OmniProvider & Model Manager, Step 3 (`POST /ai/generate`)**: implemented this
  session — see the dedicated section at the end of this file.
- **Phase 4 — OmniProvider & Model Manager, Step 4 (Google Gemini Real Execution)**: implemented
  this session — see the dedicated section at the end of this file. Backend-only, per the approved
  scope: `GeminiProvider` becomes the second real, execution-capable `OmniProvider` adapter
  (`generateText` only), following the exact pattern Step 2 established for Anthropic. No frontend
  file was touched. The first endpoint that actually
...
## Phase 4 — OmniProvider & Model Manager, Step 4 (Google Gemini Real Execution)

Approved scope (backend only, no frontend/chat UI/streaming/RAG/agents/vision/speech/billing):
integrate Google Gemini as the second real execution-capable `OmniProvider` — production-grade
adapter, DI, registration, execution support, authentication/config, error normalization, unit
tests, integration/e2e tests, docs. Follows the exact shape Phase 4 Step 2 established for
`AnthropicProvider` — no new abstraction, no change to `ModelSelectorService`, `AiService`,
`AiController`, or any frontend file.

### What changed

- **`apps/api/src/ai/providers/gemini-client.provider.ts`** (new) — the `GEMINI_CLIENT` DI token,
  a narrow `GeminiModelsClient` interface (one method: `models.generateContent`), and a
  `useFactory` Nest provider building a real `@google/genai` `GoogleGenAI` client from the
  validated `Env`. Mirrors `anthropic-client.provider.ts` for the credential-handling shape:
  `apiKey` always receives a concrete string (`env.GEMINI_API_KEY ?? "not-configured"`, never
  `undefined`). **Superseded by "Post-verification fix #2" below**: this factory only ever
  configures `httpOptions.timeout` (from `AI_REQUEST_TIMEOUT_MS`); it deliberately does not
  configure `httpOptions.retryOptions`, and `AI_MAX_RETRIES` is intentionally not applied to
  Gemini's client at all — see that section for the full root-cause trace of why. An external,
  status-aware retry loop for Gemini remains a deferred item.
- **`apps/api/src/ai/providers/gemini-error-mapper.ts`** (new) — `mapGeminiError()`, normalizing
  every failure `GeminiProvider.generateText` can encounter into the module's existing
  `AiDomainErrorCode`s. Unlike `@anthropic-ai/sdk` (a distinct exception subclass per HTTP status),
  `@google/genai` raises a single `ApiError` class and exposes only a numeric `status` — so this
  mapper branches on `status` (401/403 → `PROVIDER_AUTH_FAILED`, 429 → `PROVIDER_RATE_LIMITED`,
  400 → `PROVIDER_REQUEST_INVALID`, ≥500 → `PROVIDER_UNAVAILABLE`, any other status →
  `PROVIDER_UNAVAILABLE` as a catch-all) rather than on distinct classes. A request timeout
  surfaces as a DOM `AbortError`/`TimeoutError` (no `ApiError`, since no HTTP response was ever
  received) and is detected by `error.name` → `PROVIDER_TIMEOUT`. Never includes the SDK error's
  own `message` (which can echo back request content) in the thrown message — same "no vendor
  internal detail ever reaches a caller" guarantee Anthropic's mapper already established.
- **`apps/api/src/ai/providers/gemini.provider.ts`** (rewritten from Step 1's metadata-only stub
  into a real adapter) — `generateText` now makes a genuine `client.models.generateContent({
  model, contents: prompt, config: { maxOutputTokens: 4096 } })` call; `generateStructured`/`embed`
  remain inherited `NOT_IMPLEMENTED` throws from `StubProviderDescriptor`, so `capabilities` (and
  each model's own `capabilities`) is trimmed to `["text-generation"]` only — vision/structured-
  output/tool-calling/streaming, all advertised by Step 1's stub, are removed until a future step
  actually implements one of them, exactly as Step 2 trimmed Anthropic's. `supportsExecution()` is
  overridden to return `true` only for `"text-generation"`. The two registered models were changed
  from Step 1's placeholder `gemini-1.5-flash`/`gemini-1.5-pro` (now vendor-discontinued — every
  request to either returns a 404 as of this step) to `gemini-3.5-flash` (priority 10) and
  `gemini-2.5-pro` (priority 20), both current, generally-available, callable models as of this
  session — a real adapter must be registered against models that actually work.
  `configStatus()`/`isReady()` are unchanged (still purely `GEMINI_API_KEY`-presence-based).
- **`apps/api/src/ai/ai.module.ts`** (edited) — registers the new `geminiClientProvider` alongside
  the unchanged `anthropicClientProvider`; `GeminiProvider` is otherwise wired exactly as it was in
  Step 1 (still constructed by `AiProviderSeedService`, still exported nowhere outside this
  module). No other provider, service, or controller in this module changed.
- **`apps/api/src/ai/providers/stub-providers.spec.ts`** (edited) — `GeminiProvider` removed from
  the shared "throws `NOT_IMPLEMENTED` from every execution method" suite, for the identical reason
  `AnthropicProvider` was removed in Step 2: it no longer fits that contract or that suite's
  single-argument `new Provider(env)` construction. Only `OpenAiProvider` remains in this suite.
- **`apps/api/src/ai/providers/gemini.provider.spec.ts`** (new) — dedicated unit coverage mirroring
  `anthropic.provider.spec.ts`: metadata/capability trimming, not-configured/not-ready and
  never-leaks-a-credential guarantees, execution-eligibility (`supportsExecution`),
  `NOT_IMPLEMENTED` for `generateStructured`/`embed`, pre-execution validation (missing credential,
  unregistered model id, a foreign provider's model id — each verified to never call the client),
  a success path asserting the exact SDK request shape, whitespace trimming, empty/undefined-text
  handling, and the full SDK-error-normalization table.
- **`apps/api/src/ai/providers/gemini-error-mapper.spec.ts`** (new) — dedicated unit coverage for
  every `ApiError` status bucket, the two timeout-error-name cases, a generic network-level
  `Error`, a completely unrecognized thrown value, and a check that the raw SDK message is never
  present in the normalized response.
- **`apps/api/test/ai-generate.e2e-spec.ts`** (edited) — added a `GEMINI_CLIENT` override path
  alongside the existing `ANTHROPIC_CLIENT` one, and a new e2e case: "generates text through Gemini
  when only Gemini is configured", exercising the real `JwtAuthGuard`/`ThrottlerGuard`/
  `ZodValidationPipe`/`AiService`/`ModelSelectorService`/`ProviderRegistryService` chain end to end
  against a fake `GEMINI_CLIENT`, proving the same HTTP surface is genuinely vendor-neutral rather
  than incidentally only ever exercised against Anthropic. No existing test in this file changed
  behavior: the Anthropic-only success test still configures no `GEMINI_API_KEY`, so Gemini stays
  `not-configured`/ineligible and Anthropic is still selected, exactly as before.
- **`apps/api/package.json`** (edited) — added the `@google/genai` dependency (the current, actively
  maintained Google Gen AI SDK; the older `@google/generative-ai` package is deprecated and was
  never added).
- **`.env.example`** (edited) — documents that `GEMINI_API_KEY` now enables real execution (not
  just `configStatus()`). **Superseded by "Post-verification fix #2" below**: `AI_MAX_RETRIES` is
  documented as Anthropic-only; `AI_REQUEST_TIMEOUT_MS` remains shared by both real adapters.

### Architecture summary

`GeminiProvider` slots into the exact same `OmniProvider` interface every other adapter already
implements — `ModelSelectorService`, `ProviderRegistryService`, `AiService`, and `AiController` are
completely unchanged by this step, byte for byte. No vendor name, provider id, or model id is
hardcoded anywhere in those four files; `AiService.generate()` still requests only
`["text-generation"]` and lets `ModelSelectorService`'s existing capability/availability/readiness/
execution-eligibility algorithm decide which provider actually runs. With both `ANTHROPIC_API_KEY`
and `GEMINI_API_KEY` configured, a plain (no-preference) `POST /ai/generate` request may now be
routed to either provider depending on the selector's existing priority/registration-order
tiebreak — this is the intended, pre-existing, vendor-neutral behavior the algorithm already
specified in Step 1, not a new decision made in this step.

### Security considerations

- `GEMINI_API_KEY` is read only from the validated `Env` (never `process.env` directly), passed
  straight into the SDK constructor, and never logged, returned from any endpoint, or included in
  any normalized error message — same guarantee `gemini.provider.spec.ts`'s
  "never returns its own API key value from any public method" test enforces, matching
  `anthropic.provider.spec.ts`'s equivalent test.
- `mapGeminiError()` never surfaces the SDK's raw error `message` (which can echo back parts of
  the request) — only a fixed, generic description per normalized category, exactly as
  `mapAnthropicError()` already guarantees for Anthropic. No stack trace, response body, or header
  ever reaches a caller.
- The execution-eligibility guarantee (`supportsExecution()`, Step 3) already stopped a configured-
  but-unimplemented Gemini stub from being routed to for real execution; this step is the adapter
  Step 3's own comment anticipated ("Gemini/OpenAI before their own real adapters exist") — no
  change to that guard was needed, it already worked correctly and continues to.
- `POST /ai/generate`'s existing `JwtAuthGuard` + `@Throttle({ limit: 10, ttl: 600_000 })` apply
  identically regardless of which provider ends up selected — the route has no provider-specific
  branch of any kind.
- No prompt is ever logged by `GeminiProvider` — same as `AnthropicProvider`.

### Explicitly deferred (per approved scope)

- A chat UI, streaming, conversation history, RAG, agents, vision, speech, and billing/usage
  tracking — all out of scope per the approved scope for this step (identical to Step 3's deferred
  list; Gemini's own `generateContentStream`/multimodal/tool-calling capabilities exist in the SDK
  but are deliberately not wired up here).
- `generateStructured`/`embed` for Gemini — still `NOT_IMPLEMENTED`, tied to whichever future step
  actually implements either one (same standing note Step 2 left for Anthropic).
- A configurable `max_tokens`/`maxOutputTokens` per-request generation parameter — fixed at 4096,
  identical to Anthropic's own fixed `DEFAULT_MAX_TOKENS`.
- Any caching, request deduplication, or health/circuit-breaking for Gemini specifically — these
  remain cross-provider, dedicated future-step work, not something to bolt on per adapter.
- OpenAI remains an unimplemented Step 1 metadata-only stub — not in scope for this step.

### Verification (commands to run locally)

This session's sandbox did not have npm registry egress for a full `pnpm install` against the
newly-added `@google/genai` dependency (see Known limitations below) — the following must be run
and confirmed locally before this step is committed, per the locked workflow:
...
- `pnpm typecheck` — 15/15 turbo tasks green; `GeminiModelsClient`/`GeminiGenerateContentParams`/
  `GeminiGenerateContentResult` are structurally compatible with `@google/genai`'s own
  `GoogleGenAI`/`GenerateContentResponse` types, mirroring how `AnthropicMessagesClient` already
  relates to the `@anthropic-ai/sdk` client.
- `pnpm lint` — 15/15 turbo tasks green; no new `any`, no unused imports.
- `pnpm build` — 9/9 packages green, including `@omniscience/api` (`nest build`).
- `pnpm test` — full monorepo green. `@omniscience/api` gains: `gemini.provider.spec.ts` (new,
  ~16 cases), `gemini-error-mapper.spec.ts` (new, ~11 cases), one new e2e case in
  `ai-generate.e2e-spec.ts`, and the `stub-providers.spec.ts` suite now runs only against
  `OpenAiProvider` (its Gemini `describe.each` branch removed, not merely skipped). Every other
  package (`@omniscience/web`, `@omniscience/ui`, `@omniscience/sdk`, `@omniscience/schemas`,
  `@omniscience/config`, `@omniscience/types`, `@omniscience/utils`, `@omniscience/prompts`) is
  untouched by this step and should show identical counts to Step 3's last recorded run.

### Known limitations

- **Could not run `pnpm install`/`build`/`lint`/`typecheck`/`test` in this sandbox this session** —
  same standing network-egress constraint documented in several prior sessions (this environment's
  network allowlist for the container this work was done in did not include full npm-registry
  install access for the newly-added `@google/genai` package at the time of implementation).
  Everything above reflects careful manual review against the real `@google/genai` package's own
  published TypeScript type declarations (installed and inspected directly from the npm registry to
  confirm `GoogleGenAI`'s constructor shape, `ApiError`'s fields, and `GenerateContentResponse.text`'s
  exact undefined-safe behavior) — not a tool run. You must run the commands above locally and
  report the result before this step is committed, per the locked workflow.
- No health tracking or circuit-breaking for Gemini specifically — same standing limitation Step 2
  logged for Anthropic, deferred to a future cross-provider step.
- `gemini-3.5-flash`/`gemini-2.5-pro` are today's current, generally-available model ids as of this
  session — like any vendor's model lineup, Google may rename or retire either in the future; this
  step does not add any registry-refresh or model-discovery mechanism, matching Anthropic's own
  static, hardcoded model list.

### Post-verification fix: `mapGeminiError` no longer relies on `error instanceof ApiError`

A real local run (`ANTHROPIC_API_KEY` unset, `GEMINI_API_KEY=not-configured`) surfaced a genuine
gap: the invalid Gemini credential produced an error that did not satisfy
`error instanceof ApiError`, so `mapGeminiError` fell through to the generic
`PROVIDER_UNAVAILABLE` bucket instead of `PROVIDER_AUTH_FAILED`.

- **`gemini-error-mapper.ts`** was rewritten to classify by *structural* (duck-typed) shape instead
  of nominal class identity: `extractStatusInfo()` reads a numeric `status` directly off the thrown
  value, and — if absent — off up to five levels of `.cause` (cycle-safe), regardless of which
  concrete class produced it. This is strictly more robust than the `instanceof` check it replaces
  and still covers the nominal case (a real `ApiError` has an own `status`, found on the first,
  zero-`cause`-traversal check) — it also now tolerates the SDK's own retry layer rethrowing the
  original error wrapped in a differently-named error exposing it via `.cause`, and a
  monorepo-hoisting scenario where two structurally-identical classes end up as distinct runtime
  identities.
- A second, Gemini-specific gap was fixed alongside it: Google's backend does not reliably use
  401/403 for an invalid API key — it commonly returns a **400** whose message reads "API key not
  valid. Please pass a valid API key." A bare `status === 400` branch would have misclassified that
  as `PROVIDER_REQUEST_INVALID` even after the `instanceof` fix. A 400 is now checked (internally
  only, via `AUTH_FAILURE_MESSAGE_PATTERN`) against known auth-failure wording before falling back
  to `PROVIDER_REQUEST_INVALID` — the message itself is still never included in the response either
  way.
- **`gemini-error-mapper.spec.ts`** gained a regression suite: a duck-typed error with a foreign
  prototype, a plain object literal, a `PermanentError`-style `.cause`-wrapped `ApiError` (the
  closest reconstruction of the real failure shape), two-level `.cause` unwrapping, a cyclic-`.cause`
  safety check, and the 400-as-auth-failure quirk (both the plain-text and machine-readable
  `API_KEY_INVALID` forms), plus confirmation an ordinary 400 with no auth wording still maps to
  `PROVIDER_REQUEST_INVALID` and that the auth-detection message text is never leaked.
- No other file changed — `gemini.provider.ts`, `gemini-client.provider.ts`, `ai.module.ts`, and
  every other Step 4 file are untouched by this fix.

### Post-verification fix #2: the real root cause was `gemini-client.provider.ts`'s `retryOptions`, not the mapper

A second real local run (`ANTHROPIC_API_KEY` unset, `GEMINI_API_KEY=not-configured`) showed the
*same* `PROVIDER_UNAVAILABLE` response even after the structural-detection fix above — proving that
fix, while a genuine improvement, was not the actual root cause. Tracing the exact installed
`@google/genai@2.13.0` bundle (`dist/node/index.cjs`) end to end (rather than reasoning from the
public `.d.ts` types alone) found it: `ApiClient.apiCall()` only returns the raw HTTP `Response` —
which its caller then checks via `throwErrorIfNotOK()`, the function that builds the real,
informative `ApiError` carrying `.status` and the parsed JSON error body — when
`httpOptions.retryOptions` is **absent**. As soon as `retryOptions` is configured (which the
previous version of `gemini-client.provider.ts` did, to honor `AI_MAX_RETRIES`), `apiCall()` instead
routes every request through the SDK's bundled `p-retry@4.6.2` dependency: a non-retryable status
(e.g. 400 for a bad API key) throws `p-retry`'s own `AbortError`, which `p-retry`'s own source
(`node_modules/p-retry/index.js`) immediately unwraps via `reject(error.originalError)` — and
`originalError` is a **plain `new Error(message)`** with no `.status`, no `.cause`, and a message
containing only the bare HTTP reason phrase (e.g. `"Non-retryable exception Bad Request sending
request"`), never the actual response body. A retryable status that exhausts its retries similarly
surfaces as a plain `Error('Retryable HTTP Error: ' + statusText)`. Either way, for *every* HTTP
status — not just auth failures — the structured information the mapper needs is destroyed before
the mapper ever sees it, no matter how thoroughly it inspects the error's own shape or `.cause`
chain. (No live call to Google's API was made to observe this directly — this sandbox has no network
egress to `generativelanguage.googleapis.com` — so this was confirmed by reading the exact shipped
source of the installed `@google/genai@2.13.0` and `p-retry@4.6.2` packages, which is what actually
executes at runtime, rather than by guessing from a reconstructed fixture.)

- **`gemini-client.provider.ts`**: `httpOptions.retryOptions` removed entirely from the
  `GoogleGenAI` client construction. `httpOptions.timeout` is unaffected and still reads
  `AI_REQUEST_TIMEOUT_MS` (timeout is applied to the underlying `fetch` independently of the
  `retryOptions` branch in `apiCall()`, confirmed from the same source trace). `AI_MAX_RETRIES`
  remains defined in `Env` and still governs Anthropic's client unchanged; it is now intentionally
  *not* applied to Gemini's, since doing so is fundamentally incompatible with this SDK version
  ever surfacing a classifiable error. The trade-off (Gemini requests no longer get the SDK's own
  automatic retry-on-429/5xx) is deliberate and documented in-line and as a deferred item below —
  correct error classification is a hard requirement of this step; the SDK's internal retry
  convenience is not.
- **`gemini-error-mapper.ts`**: gained a second, defense-in-depth fallback,
  `extractStatusFromRetryWrapperMessage()`, which recognizes the two *exact* lossy message templates
  above and reverses `fetch`'s standard HTTP reason phrase back to a numeric status — confirmed
  against the real `p-retry@4.6.2` source, not guessed. This only ever recovers a bare status (the
  real response body is unrecoverable by the time `p-retry` unwraps its `AbortError`), so it cannot
  feed the 400-as-auth-message heuristic — it exists purely so a future accidental reintroduction of
  `retryOptions` degrades to a merely-generic status-based classification rather than silently
  collapsing back to `PROVIDER_UNAVAILABLE` for everything. With `retryOptions` now removed, this
  fallback is not on the primary path for any real request today.
- **`gemini-error-mapper.spec.ts`**: gained a dedicated regression suite reproducing the exact
  `p-retry`-shaped plain `Error` (no `.status`, no `.cause`) for both message templates, confirming
  400→`PROVIDER_REQUEST_INVALID` (the auth-specific body text is unrecoverable in this shape, so this
  is the correct, documented ceiling for this fallback — not a bug), 401-reason-phrase→
  `PROVIDER_AUTH_FAILED`, 429/503-reason-phrase→`PROVIDER_RATE_LIMITED`/`PROVIDER_UNAVAILABLE`, a
  non-matching message correctly falling through to the catch-all, and no reason-phrase text leaking
  into the normalized response.
- `gemini.provider.ts`, `ai.module.ts`, `gemini.provider.spec.ts`, and every other Step 4 file are
  unchanged by this fix — the real, structured `ApiError` path this file's earlier tests already
  cover (401/403/429/400/5xx, plus the 400-as-auth-message quirk) is now what genuinely executes in
  production, since `retryOptions` no longer diverts requests through the lossy path.

**New deferred item**: an external, response-status-aware retry loop for Gemini that doesn't depend
...
## Phase 5 — OmniCore, Step 1 (OmniCore Foundation)

The foundation of OmniCore, the platform's orchestration layer described in
`docs/04_System_Architecture.md`'s `User → Assistant → OmniCore → OmniProvider/Model Manager` flow.
This section documents what is already present in the repository as of this step's completion;
Steps 1–2 predate this documentation session (they were already implemented, locally verified,
committed, and pushed before Phase 5 Step 3 began), so what follows is a factual description of the
delivered code rather than a session-by-session implementation log.

### What was implemented

- **`POST /omnicore/execute`** (`apps/api/src/omnicore/omnicore.controller.ts`) — behind the same
  `JwtAuthGuard` and vendor-billed `@Throttle({ default: { limit: 10, ttl: 600_000 } })` shape as
  `POST /ai/generate`. Request body is `{ prompt: string }` only
  (`omniCoreExecuteRequestSchema`, `.strict()`); the response returns only safe routing metadata —
  `planId`, `intent`, `matchedRuleId`, `confidence`, `providerId`, `modelId`, `text` — never an
  internal plan or the matched *selector* rule.
- **`apps/api/src/omnicore/` module** — `FastRulesEngineService` (classification),
  `CapabilityPlanBuilderService` (intent → `CapabilityPlan`), `OmniCoreService` (the orchestration
  entry point composing the two), and `OmniCoreController`, registered in a new `OmniCoreModule`
  that imports `AuthModule` (for `JwtAuthGuard`) and `AiModule` (reusing `ModelSelectorService`/
  `ProviderRegistryService` exactly as Phase 4 built them — no new provider or model is registered
  by OmniCore itself).
- **OmniCore's own domain-error architecture** (`omnicore.errors.ts`) — an `OmniCoreDomainErrorCode`
  union and an `omniCoreDomainError()` helper, the same "typed error, centralized HTTP-status
  mapping" shape `ai-provider.interface.ts` already established for the AI module.
- **New shared types** (`packages/types/src/omnicore.ts`) — `OmniCoreIntent`,
  `ResolvedOmniCoreIntent`, `FastRuleMatch`, `CapabilityPlanStep`, `CapabilityPlan`,
  `OmniCoreExecuteRequest`, `OmniCoreExecuteResponse`.

### Architecture summary

`OmniCoreService.execute()` composes: classify the prompt (`FastRulesEngineService`) → build a
`CapabilityPlan` from the match (`CapabilityPlanBuilderService`) → select a model
(`ModelSelectorService.select()`) → look up its provider and call `generateText()`
(`ProviderRegistryService`). No new AI capability, provider, or model was introduced — this step is
provider-neutral routing scaffolding on top of the existing Phase 4 layer.

### Deferred items

- Real intent classification beyond a placeholder (delivered in Step 2).
- Task planning, execution orchestration, and tool calling (delivered in Steps 3–5).

## Phase 5 — OmniCore, Step 2 (Intent Intelligence)

Replaces Step 1's placeholder classification with a real, deterministic intent-classification and
capability-selection layer. Like Step 1, this section documents delivered, already-committed code
rather than a session-by-session log.

### What was implemented

- **`FastRulesEngineService`** (`apps/api/src/omnicore/fast-rules-engine.service.ts`) — a fixed
  `ResolvedOmniCoreIntent` taxonomy (`simple-generation`, `question-answering`, `code-generation`,
  `summarization`, `creative-writing`). Each rule is a hand-authored heuristic — `score()` runs
  against the trimmed prompt and returns a confidence in `[0, 1]`, with no model call anywhere in
  this file ("fast" as in "not an inference"). A confidence-margin check between the top two
  candidate rules yields a synthetic `"ambiguous"` intent (an `OmniCoreIntent`, not a
  `ResolvedOmniCoreIntent`) when a request is genuinely unclear between two or more real intents.
  An empty/whitespace-only or otherwise unscored prompt throws `INTENT_NOT_RECOGNIZED`.
- **`CapabilityPlanBuilderService`** (`apps/api/src/omnicore/capability-plan-builder.service.ts`)
  gained a real `INTENT_CAPABILITY_MAP: Record<ResolvedOmniCoreIntent, ModelCapability>` — every
  entry currently resolves to `"text-generation"`, since that remains the only capability any
  registered `OmniProvider` adapter genuinely executes (`supportsExecution()`, Phase 4 Step 3). The
  builder refuses to build a `CapabilityPlan` for an `"ambiguous"` match, throwing
  `AMBIGUOUS_INTENT` with the candidate `alternateIntents` instead.
- **`OmniCoreController`** surfaces `AMBIGUOUS_INTENT` as a `422` with `alternateIntents` in the
  body, so a genuinely unclear request is a clarification opportunity for the caller, not a guess
  made silently on their behalf.

### Architecture summary

The capability-selection seam (`INTENT_CAPABILITY_MAP`) is deliberately a lookup table, not
hardcoded logic inline in `build()` — once a specialized module or provider adapter genuinely
implements a second capability, routing an intent to it is a one-line change to the map, not a
change to the method itself. This is the same seam Step 3's `TaskPlannerService` later builds on
top of without duplicating.

### Deferred items

- Task planning, dependency-aware execution stages, execution orchestration, and tool calling
  (delivered in Steps 3–5).
- Real multi-step decomposition per intent (still not delivered as of Step 5 — every intent still
  compiles to exactly one step).

## Phase 5 — OmniCore, Step 3 (Task Planning Engine)

Approved scope: a production-grade task planning layer inside OmniCore that converts detected
intent into a structured, dependency-aware, execution-ready plan — structured task planning,
explicit dependency graph, sequential/parallel execution *planning* (not orchestration, deferred to
Step 4), complexity estimation, capability mapping, strict validation, typed domain errors, and
backward-compatible `POST /omnicore/execute` response.

### What was implemented

- **New shared types** (`packages/types/src/omnicore-plan.ts`) — `TaskComplexity`
  (`low`/`medium`/`high`/`very-high`), `StepExecutionMode` (`sequential`/`parallel`),
  `StepFailurePolicy`, `TaskPlanStep` (`stepId`, `title`, `description`, `objective`,
  `capabilities`, `toolCategory?`, `inputRequirements`, `expectedOutput`, `dependsOn`,
  `executionMode`, `complexity`, `failurePolicy`), `ExecutionStage`, `TaskPlan`.
- **`DependencyGraphService`** (`apps/api/src/omnicore/dependency-graph.service.ts`) — validates
  every step's `dependsOn` references (rejecting duplicate step ids, missing references, and a step
  naming itself) and computes a topological layering; throws `CIRCULAR_DEPENDENCY` if the graph
  cannot be fully layered.
- **`ExecutionStageBuilderService`** — turns a dependency layering into `TaskPlan.stages`: a
  single-step layer is `"sequential"`, a multi-step independent layer is `"parallel"` (safe by
  construction, since `DependencyGraphService` already guarantees no same-layer step depends on
  another).
- **`ComplexityEstimatorService`** — classifies per-step and per-plan complexity from plan shape
  alone (capability count, tool presence, dependency depth, stage count, whether any stage is
  parallel) — no model call.
- **`PlanValidatorService`** — structural validation: non-empty plan, every step has at least one
  capability, every capability is one this phase actually supports
  (`UNSUPPORTED_CAPABILITY_MAPPING` otherwise).
- **`TaskPlannerService`** — the seam that builds a `TaskPlan` from an existing `CapabilityPlan`,
  reusing (not duplicating) `CapabilityPlanBuilderService`'s own capability-selection logic. Every
  resolved intent still compiles to exactly one `TaskPlanStep` today, so every `TaskPlan` OmniCore
  actually produces end to end is a single-step, single-stage plan — the multi-step/parallel
  machinery is exercised directly against hand-built fixtures in each service's own unit tests.
- **Six new domain error codes** (`omnicore.errors.ts`, all `422`): `INVALID_TASK_PLAN`,
  `CIRCULAR_DEPENDENCY`, `MISSING_DEPENDENCY`, `DUPLICATE_STEP_ID`,
  `UNSUPPORTED_CAPABILITY_MAPPING`, `PLANNING_FAILED`. None are reachable through the public
  `POST /omnicore/execute` request today (a caller can never submit their own plan or steps) — they
  are defensive guards a correctly restructured planner must still satisfy.
- **`OmniCoreExecuteResponse`** gained an additive `taskPlan: TaskPlan` field. Every field Step 1/2
  already returned is unchanged in name, type, or meaning.

### Architecture summary

`OmniCoreService.execute()`'s flow became: classify → capability plan → **task plan** → select →
execute. The task plan is built before model selection, so a planning failure is surfaced without
ever making a real, vendor-billed provider call.

### Deferred items

- Actual orchestration of a `TaskPlan`'s stages/steps (delivered in Step 4).
- Real multi-step decomposition per intent (not yet delivered).

### Verification (commands run in-sandbox this session)
...
- `pnpm typecheck` / `pnpm lint` / `pnpm build` — all green across all 9 packages.
- `pnpm test` — 15/15 turbo tasks green, full monorepo; `@omniscience/api`: **60 suites / 560
  tests** (up from Phase 4 Step 5's 465).

## Phase 5 — OmniCore, Step 4 (Execution Orchestration Engine)

Approved scope: an orchestration layer that consumes a `TaskPlan` and executes its stages and steps
safely, deterministically, and observably — sequential/parallel execution, dependency enforcement,
strongly typed execution state, the `"abort"` failure policy (architected so `"continue"`/`"retry"`
can be added later without a redesign), typed execution results, in-process cancellation/timeouts,
OmniCore integration, structured observability, and typed domain errors.

### What was implemented

- **New shared types** (`packages/types/src/omnicore-execution.ts`) — `ExecutionStatus`
  (`pending`/`running`/`completed`/`failed`/`skipped`/`cancelled`), `StepExecutionResult`,
  `StageExecutionResult`, `PlanExecutionResult`.
- **`StepExecutorService`** (`apps/api/src/omnicore/step-executor.service.ts`) — runs one
  `TaskPlanStep` through the existing `ModelSelectorService`/`ProviderRegistryService` path (the
  same call `OmniCoreService` used to make directly before this step); races the provider call
  against an optional timeout (`EXECUTION_TIMEOUT`) and an optional `AbortSignal`
  (`EXECUTION_CANCELLED`).
- **`ExecutionOrchestratorService`** — walks `TaskPlan.stages` in order: `"sequential"` stages run
  one step at a time, `"parallel"` stages run concurrently via `Promise.all`. Re-validates
  dependencies through Step 3's `DependencyGraphService` and defensively re-checks each step's
  `dependsOn` before running it (`DEPENDENCY_FAILURE`, `INVALID_EXECUTION_STATE`). Only the
  `"abort"` failure policy is implemented: the first step failure, timeout, or cancellation stops
  the whole plan and lets the original, already-typed error propagate unchanged after being logged
  — architected so a future `"continue"`/`"retry"` policy only changes that one branch, not the
  result-building.
- **`OmniCoreExecuteResponse`** gained a second additive field, `execution: PlanExecutionResult`,
  alongside the untouched `taskPlan`.
- **`OmniCoreService`** now composes: classify → capability plan → task plan → **orchestrate**,
  deriving `text`/`providerId`/`modelId` from the orchestrator's single step result instead of
  calling the selector/registry directly.
- **Eight new domain error codes** (all `422` except `EXECUTION_TIMEOUT`, `408`):
  `UNSUPPORTED_CAPABILITY`, `DEPENDENCY_FAILURE`, `INVALID_EXECUTION_STATE`,
  `PLAN_EXECUTION_FAILED`, `STAGE_EXECUTION_FAILED`, `STEP_EXECUTION_FAILED`, `EXECUTION_TIMEOUT`,
  `EXECUTION_CANCELLED`.

### Architecture summary

Cancellation and timeouts are in-process only (an `AbortController`/`AbortSignal` plus an optional
`timeoutMs`, raced against the provider call) — no persistent job management, no distributed
workers or queues, per the approved constraints.

### Deferred items

- `"continue"`/`"retry"` failure policies.
- Distributed workers, queues, persistent execution storage, multi-agent coordination.
- Real multi-step decomposition per intent (still not delivered).
...
- `pnpm typecheck` / `pnpm lint` / `pnpm build` — all green across all 9 packages.
- `pnpm test` — 15/15 turbo tasks green, full monorepo; `@omniscience/api`: **62 suites / 580
  tests** (up from Step 3's 560).

## Phase 5 — OmniCore, Step 5 (Tool Calling Framework)

Approved scope: a production-grade tool execution framework so OmniCore can safely invoke internal
tools as part of execution plans — the foundation for future RAG, search, memory, web, and database
integrations, none of which are built in this step. A generic `Tool` contract, a tool registry, a
tool executor (input validation, output capture, execution metadata, timeout/cancellation), three
foundation built-in tools, integration into `StepExecutorService`, typed tool-result models,
observability, and typed domain errors.

### What was implemented

- **`Tool<TInput, TOutput>`** (`apps/api/src/omnicore/tools/tool.interface.ts`) — the generic
  contract every tool implements: `id`, `name`, `description`, `capabilities`, `inputSchema`,
  `outputSchema`, `execute()`. `ToolSchema<T>` is structurally typed (a bare `safeParse` method,
  "ZodLike" rather than a direct `zod` dependency) — the same convention `ZodValidationPipe`
  established for HTTP request bodies. Includes small shared helper schemas (`predicateSchema`,
  `anyInputSchema`, `stringSchema`).
- **`ToolRegistryService`** — register/`getById`/`tryGetById`/`list`, mirroring
  `ProviderRegistryService`; throws `TOOL_NOT_FOUND` for an unregistered id, `DUPLICATE_TOOL_ID` for
  a duplicate registration.
- **`ToolExecutorService`** — validates input against the tool's `inputSchema`
  (`INVALID_TOOL_INPUT` otherwise) → runs the tool, raced against an optional timeout
  (`TOOL_TIMEOUT`) and cancellation signal (`TOOL_CANCELLED`) via a shared
  `execution-timeout.util.ts` extracted from Step 4's `StepExecutorService` (so both reuse the
  identical race logic) → validates the result against the tool's `outputSchema`
  (`TOOL_EXECUTION_FAILED` otherwise) → returns a typed `ToolExecutionResult`. Never double-wraps an
  already-typed `HttpException` a tool throws itself; wraps any other thrown value as
  `TOOL_EXECUTION_FAILED`.
- **Three built-in tools** (`apps/api/src/omnicore/tools/built-in/`) — `EchoTool` (returns its
  input unchanged), `CurrentTimeTool` (returns the current UTC ISO 8601 timestamp), `UUIDTool`
  (generates a random v4 UUID via `node:crypto`'s `randomUUID`).
- **`ToolSeedService`** (`OnModuleInit`) — registers all three built-in tools into
  `ToolRegistryService` exactly once at bootstrap, mirroring `AiProviderSeedService`.
- **`StepExecutorService` integration** — now routes on `TaskPlanStep.toolCategory` (a field that
  already existed since Step 3): set → `ToolExecutorService`; unset → the existing model path,
  unchanged. No real `TaskPlan` sets `toolCategory` yet (`TaskPlannerService` never produces one),
  so tool-routed step execution is exercised directly via unit tests — the same "generic machinery
  ahead of its real producer" precedent Steps 3/4 already established.
- **New shared types** (`packages/types/src/tool.ts`) — `ToolCapability` (a plain `string`,
  deliberately open unlike the closed `ModelCapability` union), `ToolMetadata`,
  `ToolExecutionResult`. `StepExecutionResult` gained an additive optional `toolId` field.
- **Six new domain error codes**: `TOOL_NOT_FOUND` (`404`), `DUPLICATE_TOOL_ID` (`409`),
  `INVALID_TOOL_INPUT` (`400`), `TOOL_EXECUTION_FAILED` (`422`), `TOOL_TIMEOUT` (`408`),
  `TOOL_CANCELLED` (`422`).

### Architecture summary

Mirrors the AI module's own provider layer split: the descriptive metadata (`ToolMetadata`) is a
shared type, the executable contract (`Tool`, with `execute()`) lives in `apps/api` next to the
NestJS code that implements and consumes it — the same shape `ModelMetadata`/`OmniProvider` already
established. `StepExecutorService` is the only consumer of `ToolExecutorService`; nothing else in
`OmniCoreService` or the module needed to change to support tool-routed steps.

### Deferred items

- External APIs, web search, RAG, databases, memory, and MCP integrations — this step only builds
  the generic tool framework and three foundation tools.
- Any real `TaskPlan` producer that sets `toolCategory` (still not delivered).
...
- `pnpm typecheck` / `pnpm lint` / `pnpm build` — all green across all 9 packages.
- `pnpm test` — 15/15 turbo tasks green, full monorepo; `@omniscience/api`: **69 suites / 624
  tests** (up from Step 4's 580).

This is Phase 5 Step 5. No further step or phase has been started.
...
This was the current-priority snapshot as of the end of the post-Phase-B bugfix pass, before Step 4
began:
...
**This snapshot is superseded — see the "Phase 6 — Omniscience Assistant, Step 4 (Conversation
Management)" section immediately below for what has happened since, and the final "Current
development priority remains" banner at the very end of this file for the actual current status.**
...
**Verification — in-sandbox (this session, environment-limited):** `pnpm build`, `pnpm lint`,
`pnpm typecheck` all green across all 9 packages. `pnpm test`: API 75 suites / 759 tests passed;
web 26 suites / 169 tests passed; SDK 59 tests passed; schemas 123 tests passed. The real-Mongo
half of `conversations.repository.spec.ts` skipped in-sandbox (no local MongoDB/Docker available
here) — a sandbox-only limitation, not something this step introduced.

**Verification — user's local machine (authoritative for anything the sandbox couldn't run):**
same build/lint/typecheck/test results reproduced locally, **plus the real-Mongo repository spec
run for real: 19/19 tests passing**, the one suite the sandbox was never able to exercise. An
intermediate local run had hit 19/19 failures in that same suite — all-or-nothing across all 19
tests, including the 13 pre-existing Step 1/2 tests Step 4 never touched, which pointed at a
`beforeAll`/environment-level failure rather than a Step 4 logic bug (the suite's own
`FakeMongoService`-independent design, and the separately-passing e2e suite already proving the new
`FakeMongoService` methods correct, ruled out a Step 4 code defect). That was root-caused as a
local MongoDB/environment setup issue on the user's machine and resolved without any application
code change. The user also manually verified real browser behavior: rename works and persists
after refresh; delete works and stays deleted after refresh; deleting the currently-open
conversation correctly clears back to a fresh/no-active-conversation state.
...
**Commit status:** **COMPLETE.** Implementation, local verification (build/lint/typecheck/full test
suite all green on the user's own machine, including the real-Mongo repository spec passing 19/19
for real), manual browser QA, commit, and push are all done, and the user has explicitly confirmed
the push. Per this project's locked workflow (local verification → explicit approval → git commit
→ git push → user confirmation of the push), all five have now happened — this step is officially
complete.

---

## Phase 6 — Omniscience Assistant, Step 5 (Message-Level UX)

**Context:** confirmed via re-investigation (not assumed from the prior planning pass) that
message-level copy already existed, fully implemented and tested, in `MessageBubble.tsx` since
before Step 4 — an undocumented pre-existing Step-3-era feature, the same class of gap as the
`MarkdownMessage.tsx` doc gap. It was **not** reimplemented; both remain flagged for Step 6
documentation reconciliation. The genuinely new work this step is the guarded last-message-only
delete primitive, and the regenerate/edit-and-resend frontend flows built on top of it.

**Backend:** `conversations.errors.ts` gained `MESSAGE_NOT_FOUND` (404, same no-enumeration
convention as `CONVERSATION_NOT_FOUND`) and `MESSAGE_NOT_LAST` (409 — the message exists and is the
caller's, but isn't currently the conversation's last message — a real state conflict, not a
not-found). `ConversationsRepository` gained `getLastMessage()` (same `createdAt`+`_id` ordering
`listMessages()` already uses) and `deleteMessage()`, which independently re-derives the true last
message from the database on every call — never trusts the caller's claim — and only deletes when
it matches. `ConversationsService.deleteLastMessage()` resolves ownership via
`getOwnedConversationOrThrow()` first, exactly like every other conversation-scoped method, then
maps the repository's three-way outcome to the two domain errors. `ConversationsController` gained
`DELETE :conversationId/messages/:messageId`, same guard/throttle/validation conventions as every
other conversation write. No new domain error for "arbitrary message deletion" was introduced —
there is deliberately no general delete-any-message route, only this one guarded primitive.

**Frontend:** `useMessageStream.ts` gained a `remove_trailing` reducer action (the reducer
previously only ever appended or mutated messages in place) and two new methods:
`regenerateLastAssistantMessage()` deletes the true-last assistant message via the guarded
primitive and, only on success, resends the immediately-preceding user message's actual content
(read live from current state, not from the potentially-stale `lastSentContentRef`) through the
completely unmodified `sendMessage()`/`sendMessageStream()` path — no new send endpoint, no changes
to the streaming implementation. `editLastUserMessage(newContent)` deletes the trailing assistant
reply first if one exists, then the user message itself, then resends — both delete steps
independently guarded the same way. A failed delete at any point never removes the message locally
and never triggers a resend; the failure surfaces through the existing `streamError`/
`getChatErrorMessage` mechanism, unchanged. `MessageList.tsx` computes, purely from its own
`messages` array, which single message (if any) is eligible for each action — display-only
conveniences that can never cause an unintended deletion, since the backend independently
re-verifies on every call. `MessageBubble.tsx` gained a "↻ Regenerate" action (assistant bubbles,
next to the pre-existing Copy) and inline "✎ Edit" editing (user bubbles) mirroring
`ConversationSidebar`'s existing inline-rename interaction — Save/Enter commits, Cancel/Escape
restores, an emptied-out edit is treated as a cancel. `ConversationThread.tsx` needed only two new
callback props threaded through — no structural change.

**Implementation note (not a limitation — the approved design working as specified):** regenerate
resends through the ordinary `sendMessage()` path, which always creates a genuinely new user
message via the normal send flow — so after a regenerate, the conversation gains a third message
(original user message, a new resent copy, new assistant reply), not two; only the old assistant
reply is actually removed. Edit-and-resend doesn't have this effect, since it also deletes the old
user message before resending, leaving exactly two messages behind.

**Accepted limitation (confirmed, not architected around):** delete-then-resend is not
transactional — the standalone (non-replica-set) MongoDB deployment can't provide multi-document
transactions, and one couldn't span the non-transactional external AI-provider resend call anyway.
If a delete succeeds and the resend then fails, the deleted message(s) stay deleted and the user
retypes/resends. No replica-set migration, distributed transactions, version trees, branching, or
recovery infrastructure were introduced to solve this.

**Two real bugs found and fixed during this step's own verification, before being marked
"implementation verified" (both already fixed — recorded for the historical record):**
1. An accessible-name collision in `MessageBubble.tsx` — the inline edit `Input` and the "Edit"
   trigger button both initially shared the aria-label `"Edit message"`, so a query for the input
   by that label could instead match the button once editing closed. Fixed by giving the input its
   own distinct label, `"Edit message content"`.
2. A wrong test expectation, not a code bug, in `useMessageStream.test.ts` — the first draft
   asserted 2 messages after a regenerate; per the "Implementation note" above, 3 is correct. Fixed
   the test, not the implementation.

Also caught and fixed one incorrect e2e test assumption: a test asserted `MESSAGE_NOT_FOUND` for a
delete attempted by a caller who doesn't own the conversation at all, but the actual, correct,
secure behavior is `CONVERSATION_NOT_FOUND` — the top-level ownership check on the conversation
rejects first, before anything message-specific is ever reached, exactly mirroring how
`rename`/`remove` already behave. Fixed the test, not the implementation.

**Verification (in-sandbox, this session):** `pnpm build`, `pnpm lint`, `pnpm typecheck` all green
across all 9 packages. `pnpm test`: schemas 129 tests passed; SDK 62 tests passed; web 26 suites /
197 tests passed; API — the four `conversations.*` spec/e2e files (28 + 48 + 40 = 116 tests, real-
Mongo half of the repository spec still skipping in-sandbox, same pre-existing environment-only gap
as Steps 1–4) plus every other API suite (ai/auth/omnicore/users/workspaces/avatar/common/config/
health/mail/mongo/prisma/redis unit suites, and the account-deletion/ai-generate/health/
auth-registration/avatar/session-management/users-profile/workspaces e2e suites) all passed, no
unexplained failures anywhere.

**Files changed:** `apps/api/src/conversations/conversations.errors.ts`, `.repository.ts` (+spec),
`.service.ts` (+spec), `.controller.ts` (+spec), `apps/api/test/conversations.e2e-spec.ts`,
`packages/schemas/src/conversations.ts` (+test), `packages/schemas/src/index.ts`,
`packages/types/src/conversations.ts`, `packages/types/src/index.ts`, `packages/sdk/src/client.ts`
(+test), `apps/web/src/features/chat/useMessageStream.ts` (+test), `MessageList.tsx` (+test),
`MessageBubble.tsx` (+test), `ConversationThread.tsx` (callback wiring only). `MessageBubble.tsx`'s
pre-existing Copy button was not touched.

**Strictly out of scope, confirmed untouched:** `ModelSelectorService`, `GeminiProvider`,
`gemini-error-mapper.ts`, provider priorities/registration order/fallback behavior, and any
OmniCore provider-selection architecture — the separately-investigated Gemini upstream-availability
issue does not enter this step.

---

**Critical bugfix round — user's real browser QA found 2 real implementation bugs.** Before any
commit, the user's own manual QA of the round above found two genuine, reproducible bugs neither
in-sandbox verification pass had caught, since neither manifests until a conversation crosses the
20-message page-size boundary:

1. **New messages "disappear" after a refresh, navigating away, or reopening the conversation.**
2. **Edit/Regenerate don't work on pre-existing (longer) conversations, though they work on
   freshly-created ones.**

Both traced to one shared root cause, a **pre-existing Step 3 gap** (not introduced by Step 5):
`ConversationThread.loadHistory()` called `listMessages()` exactly once with no `cursor` and never
followed `nextCursor`. The backend's `listMessages` is bounded and cursor-paginated, oldest-first
(`DEFAULT_MESSAGE_LIST_LIMIT = 20`), so any conversation past 20 messages only ever showed its
*oldest* 20 — new messages were persisted correctly server-side the whole time, just never fetched
(Issue 1), and `MessageList`'s "is this the true last message" eligibility, computed from that
truncated list, disagreed with the server's actual last message, so the guarded delete correctly
(but confusingly) rejected with `409 MESSAGE_NOT_LAST` (Issue 2) — explaining exactly why fresh,
short conversations worked and older, longer ones didn't.

**Fix 1 — full-history pagination:** `ConversationThread.tsx`'s `loadHistory()` now loops on
`nextCursor` (requesting `MAX_MESSAGE_LIST_LIMIT` (50) per page) until exhausted, capped by a
defensive `MAX_HISTORY_PAGES = 10` circuit breaker against an unforeseen server-side cursor bug.
Chronological ordering is preserved and there are no duplicates (the cursor's `$gt` filter strictly
excludes already-seen ids); existing loading/error UI states were untouched.

**Fix 2 — URL-based conversation-selection persistence:** the reported "refresh navigates back to
the conversation list" behavior was a separate, additional pre-existing gap
(`activeConversationId` lived only in React state) the user asked to have investigated alongside
Issue 1. Added an optional `:conversationId` route segment in `App.tsx`; `ChatPage.tsx` reads it and
passes it to `ChatPanel` as `initialConversationId`; `ChatPanel.tsx` keeps the URL in sync with the
live selection via `navigate(..., { replace: true })` on every select/create/clear, so a hard
refresh now restores the conversation that was open.

**Files changed this round:** `apps/web/src/features/chat/ConversationThread.tsx` (+test),
`apps/web/src/App.tsx` (new route), `apps/web/src/pages/ChatPage.tsx`,
`apps/web/src/features/chat/ChatPanel.tsx` (+test). Two now-stale pre-existing test assertions in
`ChatPanel.test.tsx` (strict `listMessages` call-argument checks) were loosened to accommodate the
new query parameter `listMessages` always passes now.

**Re-verification after this round:** `pnpm build`/`lint`/`typecheck` all green again across all 9
packages; web 26 suites / 201 tests passed (4 net-new); the conversations backend suites, schemas,
and SDK were re-run and remain green — none of this round's changes touched backend/SDK/schemas
code.

---

**Commit status:** implementation complete, including this bugfix round; full in-sandbox
verification green, including the critical bugfix round above. Per this project's locked workflow
(implementation → Claude's own verification → ZIP handoff → user's local replace/QA → commit →
push → user's explicit push confirmation), the user's own local QA has now passed: full
conversation history persists correctly; reload no longer loses newly-created messages; refresh
restores the currently-open conversation; navigation between conversations no longer loses state;
the latest user message can be edited; the latest assistant message can be regenerated; older
messages intentionally cannot be edited/regenerated (matches the approved last-turn-only scope);
copy still works correctly. No implementation issues remain. **This step is officially COMPLETE.**
Step 6 is now the active/current step — not yet started, awaiting explicit go-ahead before
implementation begins (see `claude/PHASE_PLAN.md`'s Step 6 section for approved scope).
...
> and verified as described above; Step 4 — Conversation Management — COMPLETE (implementation,
> local verification, commit, and push all done and confirmed), see immediately above; Step 5 —
> Message-Level UX — COMPLETE (implementation, full verification, the critical bugfix round, and
> the user's local QA all done and confirmed), see immediately above; Step 6 — Phase Close-Out —
> is now the active/current step, a reconstructed, approved plan not yet implemented; live
> browser QA of the Phase B scroll fixes and real-session soak testing of the token refresh from
> the post-Phase-B bugfix pass remain pending a maintainer with a browser and network access,
> tracked for Step 6 close-out)**
...
> **Phase 6 — Omniscience Assistant (Steps 1–3, Phase B, and the post-Phase-B bugfix pass complete and verified; Step 4 — Conversation Management — COMPLETE (implementation, local verification, commit, and push all done and confirmed); Step 5 — Message-Level UX — COMPLETE (implementation, full verification, the critical bugfix round, and the user's local QA all done and confirmed); Step 6 — Phase Close-Out — is now the active/current step, a reconstructed, approved plan not yet implemented; live browser QA of the Phase B scroll fixes and real-session soak testing of the token refresh remain pending a maintainer with a browser and network access, tracked for Step 6 close-out)**
